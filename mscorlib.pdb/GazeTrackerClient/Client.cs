...
namespace GazeTrackerClient
{
  ...
  public class Client
  {
    ...
    public void Connect()
    {
      ...
      {
        this.Disconnect();
        var errorMessage = "Errore di connessione all'eyetracker. Messaggio: " + ex.Message;
        throw new Exception(errorMessage, ex);
      }
    }

    /// <summary>
    /// Restarts the client by connecting to a running gazetracker instance,
    /// which has the network options enabled.
    /// </summary>
    ...
    public void Disconnect()
    {
      ...
      {
        this.IsRunning = false;
        var errorMessage = "Impossibile terminare la connessione. Messaggio: " + ex.Message;
    ...
    public bool SendCommand(string cmd, string value)
    {
      ...
      {
        var errorMessage = "Errore durante l'invio di un comando all'eyetracker. Messaggio: " + ex.Message;
        this.OnErrorOccured(new StringEventArgs(errorMessage));
        this.OnClientConnectionChanged(false);
    ...
    private void StartListen()
    {
      ...
      try
      {
        ...
        {
          var errorMessage = "Errore di connessione. Messaggio: " + e.Message;
          this.OnErrorOccured(new StringEventArgs(errorMessage));
          this.OnClientConnectionChanged(false);
          return;
        }
        ...
        while (this.IsRunning)
        {
          ...
          {
            this.OnClientConnectionChanged(false);
            var errorMessage = "Errore in ricezione dati. Messaggio: " + ex.Message;
            this.OnErrorOccured(new StringEventArgs(errorMessage));
          }
      ...
      {
        var errorMessage = "Errore al socket CA. Messaggio: " + ex.Message;
        this.OnErrorOccured(new StringEventArgs(errorMessage));
        this.OnClientConnectionChanged(false);
      }
    }
